...
int main(int argc, char const *argv[]) {
	...
	if(argc < 2) {
      cerr << "Usage: " << argv[0] << " /path/to/photo/" << endl;
      cout << "Exiting...." << endl;
      return -1;
    }
    ...
    //Mat immag = imread(filename, CV_LOAD_IMAGE_COLOR | CV_LOAD_IMAGE_ANYDEPTH);


    // mat container to receive images
    Mat source, destination;

    source = imread(filename, CV_LOAD_IMAGE_COLOR | CV_LOAD_IMAGE_ANYDEPTH);
